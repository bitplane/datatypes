...
static void expect_rows(const struct fax_image *image, const char *const *rows, unsigned count)
{
    ...
    for (y = 0; y < count; y++) {
        ...
        for (x = 0; x < image->width; x++)
            assert(fax_pixel(image, x, y) == (rows[y][x] == 'X'));
    }
}
...
static void test_g3(void)
{
    ...
    /* Long runs use make-up codes, including the extended ones. */
    {
        ...
        assert(fax_pixel(&wide, 64, 0) == 0 && fax_pixel(&wide, 65, 0) == 1);
        assert(fax_pixel(&wide, 65 + 1729, 0) == 1 && fax_pixel(&wide, 65 + 1730, 0) == 0);
        assert(fax_pixel(&wide, wide.width - 1, 0) == 0 && wide.stride == (wide.width + 7u) / 8u);
        fax_free(&wide);
    }
    ...
    /* 1100 lines of white 65535 (25 x 2560 + 1472 + 63) is 72M pixels. */
    {
        static const char *const parts[] = {EOL, "000000011111", "010011000" "00110100"};
        char *bits = malloc(1100u * (12u + 25u * 12u + 17u) + 80u), *end;
        size_t i, j;
        uint8_t *data;
        assert(bits != NULL);
        end = bits;
...
static void test_g4(void)
{
    ...
    assert(fax_decode_g4(buffer, size, 8193, 8193, &image) == CODEC_TOO_LARGE);
                               ...
                               int transpose, int flip_x, int flip_y)
{
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            assert(fax_pixel(&image, x, y) == (g4_rows[sy][sx] == 'X'));
        }
    }
    fax_free(&image);
}
...
static void test_cals(void)
{
    ...
    /* The pel path counter-clockwise from rightwards, then the line
       progression counter-clockwise from the pel path. */
    expect_orientation("000,270", 8, 10, 0, 0, 0);
    expect_orientation("000,090", 8, 10, 0, 0, 1);
    expect_orientation("180,270", 8, 10, 0, 1, 1);
    expect_orientation("180,090", 8, 10, 0, 1, 0);
    /* Pels upwards, lines rightwards: turned a quarter anticlockwise. */
    expect_orientation("090,270", 10, 8, 1, 0, 1);
    expect_orientation("090,090", 10, 8, 1, 1, 1);
    expect_orientation("270,270", 10, 8, 1, 1, 0);
    expect_orientation("270,090", 10, 8, 1, 0, 0);
    expect_orientation("0,270", 8, 10, 0, 0, 0);
    /* Nonsense orientations are ignored. */
    expect_orientation("000,000", 8, 10, 0, 0, 0);
    expect_orientation("000,180", 8, 10, 0, 0, 0);
    expect_orientation("090,000", 8, 10, 0, 0, 0);
    expect_orientation("045,315", 8, 10, 0, 0, 0);
    expect_orientation("360,270", 8, 10, 0, 0, 0);
    ...
    /* Any header record may come first. */
    {
        static const char *const first[] = {
            "dstdocid: NONE", "txtfilid: NONE", "figid: NONE", "srcgph: NONE",
            "doccls: UNCLASS", "rtype: 1", "rpelcnt: 8,10", "rdensty: 0200", "notes: NONE",
        };
        size_t i;
        for (i = 0; i < sizeof first / sizeof first[0]; i++) {
            const char *records[] = {first[i], "rpelcnt: 8,10", NULL};
            assert(fax_is_cals(buffer, cals(buffer, records, G4_ROWS)));
            assert(fax_decode(buffer, cals(buffer, records, G4_ROWS), &image) == CODEC_OK);
            expect_rows(&image, g4_rows, 10);
            fax_free(&image);
        }
    }
    /* An E-size drawing at 200 dpi, all white (V0 on every row), stored sideways. */
    {
        static char bits[6800 + 1];
        const char *records[] = {"srcdocid: NONE", "rtype: 1", "rorient: 090,270",
                                 "rpelcnt: 008800,006800", NULL};
        memset(bits, '1', 6800);
        size = cals(buffer, records, bits);
        assert(fax_decode(buffer, size, &image) == CODEC_OK);
        assert(image.width == 6800 && image.height == 8800 && image.stride == 850);
        for (size = 0; size < image.stride * image.height; size++)
    ...
    /* Errors. */
    {
        ...
        const char *big[] = {"srcdocid: NONE", "rpelcnt: 8193,8193", NULL};
    ...
    memset(buffer, ' ', 128);
    memcpy(buffer, "rpelcnt 8,10", 12);
    assert(fax_is_cals(buffer, 128) == 0);
    memcpy(buffer, "version: 1", 10);
    assert(fax_is_cals(buffer, 128) == 0);
}
...
static void test_encode(void)
{
    ...
    /* Threshold and compositing over white. */
    {
        ...
        for (i = 0; i < sizeof cases / sizeof cases[0]; i++) {
            ...
            assert(image.width == 1 && fax_pixel(&image, 0, 0) == cases[i].black);
            fax_free(&image);
        }
    }
    fax_encoder_init(&encoder);
       ...
       ignored, in either order. */
    {
        ...
        for (pass = 0; pass < 2; pass++) {
            ...
            assert(memcmp(image.pixels, forward.pixels, 25u * 40u) == 0);
            fax_free(&image);
            reverse_bytes(stream, total);
            assert(fax_decode(stream, total, &image) == CODEC_OK);
            assert(image.width == 200 && image.height == 40);
       ...
       alternation, and random rows. */
    {
        ...
        static uint8_t stream[1 << 16], expected_rows[5][7000];
        ...
        for (w = 0; w < sizeof widths / sizeof widths[0]; w++) {
            ...
            for (y = 0; y < height; y++) {
                for (x = 0; x < width; x++) {
                    ...
                    expected_rows[y][x] = (uint8_t)black;
                }
            ...
            for (y = 0; y < height; y++)
                for (x = 0; x < width; x++)
                    assert(fax_pixel(&image, x, y) == expected_rows[y][x]);
            fax_free(&image);
