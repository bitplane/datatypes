...
#define MAX_XWD_FILE (128L * 1024L * 1024L)
...
static LONG load_xwd(Class *cl, Object *obj)
{
    struct xwd_image image;
    UBYTE *input;
    LONG size, error;

    error = dt_read_file(obj, 8, MAX_XWD_FILE, &input, &size);
    if (error != 0 || input == NULL)
        return error;
    error = dt_error(xwd_decode(input, (size_t)size, &image));
    FreeVec(input);
    if (error != 0)
        return error;
...
static BOOL write_row(void *state, const UBYTE *rgba, ULONG width)
{
    ...
    size_t size = xwd_encode_row(rgba, width, w->out, w->capacity);
    return size != 0 && dt_write(w->file, w->out, (LONG)size);
}

IPTR XWD__OM_NEW(Class *cl, Object *obj, struct opSet *msg)
{
    return dt_new(cl, obj, msg, load_xwd);
}

IPTR XWD__DTM_WRITE(Class *cl, Object *obj, struct dtWrite *msg)
{
    UBYTE header[XWD_WRITE_HEADER];
    struct writer w;
    ULONG width, height;
    IPTR success;
    ...
    if (!dt_picture_size(obj, &width, &height) ||
        !xwd_make_header(width, height, header))
        return FALSE;
