...
/* Room for 16M 24-bit pixels, a full CLUT and several images back to back. */
#define MAX_TIM_FILE (256L * 1024L * 1024L)
...
static LONG load_tim(Class *cl, Object *obj, ULONG index, ULONG *count)
{
    ...
    error = dt_read_file(obj, 8, MAX_TIM_FILE, &input, &size);
    if (error != 0 || input == NULL)
        return error;
    images = tim_count(input, (size_t)size);
    if (count != NULL)
        *count = images;
    ...
    error = dt_error(tim_decode(input, (size_t)size, (unsigned)index, &image));
    FreeVec(input);
    if (error != 0)
        return error;
...
static BOOL write_row(void *state, const UBYTE *rgba, ULONG width)
{
    ...
    tim_encode_row(rgba, width, w->out);
    return dt_write(w->file, w->out, w->size);
}

IPTR Tim__OM_NEW(Class *cl, Object *obj, struct opSet *msg)
{
    ...
    if (created == 0)
        return 0;
    error = load_tim(cl, (Object *)created, index, count);
    if (error != 0) {
...
IPTR Tim__DTM_WRITE(Class *cl, Object *obj, struct dtWrite *msg)
{
    UBYTE header[20];
    struct writer w;
    ULONG width, height;
    ...
    if (!dt_picture_size(obj, &width, &height) ||
        !tim_make_header(width, height, header))
        return FALSE;
    w.file = msg->dtw_FileHandle;
    w.size = (LONG)tim_row_size(width);
