...
/* Room for padding and a MacBinary resource fork. */
#define MAX_MACPAINT_FILE (1024L * 1024L)
...
struct writer {
    BPTR file;
    ULONG row;
    UBYTE out[MACPAINT_ROW_MAX];
};

static LONG put_bitmap(Class *cl, Object *obj, const struct macpaint_image *image)
{
...
static LONG load_macpaint(Class *cl, Object *obj)
{
    struct macpaint_image image;
    UBYTE *input;
    LONG size, error;

    error = dt_read_file(obj, MACPAINT_HEADER_SIZE, MAX_MACPAINT_FILE, &input, &size);
    if (error != 0 || input == NULL)
        return error;
    error = dt_error(macpaint_decode(input, (size_t)size, &image));
    FreeVec(input);
    if (error != 0)
        return error;
...
/* MacPaint pictures are always 576x720: keep the top left of larger
   pictures and pad smaller ones with white. */
static BOOL write_row(void *state, const UBYTE *rgba, ULONG width)
{
    struct writer *w = state;
    size_t size;

    if (w->row >= MACPAINT_HEIGHT)
        return TRUE;
    w->row++;
    size = macpaint_encode_row(rgba, width, w->out, sizeof w->out);
    return size != 0 && dt_write(w->file, w->out, (LONG)size);
}

IPTR MacPaint__OM_NEW(Class *cl, Object *obj, struct opSet *msg)
{
    return dt_new(cl, obj, msg, load_macpaint);
}

IPTR MacPaint__DTM_WRITE(Class *cl, Object *obj, struct dtWrite *msg)
{
    UBYTE header[MACPAINT_HEADER_SIZE];
    struct writer w;
    ULONG width, height;
    ...
    if (!dt_write(w.file, header, sizeof header) ||
        ...
        return FALSE;
    while (w.row < MACPAINT_HEIGHT)
        if (!write_row(&w, NULL, 0))
            return FALSE;
    return TRUE;
}
